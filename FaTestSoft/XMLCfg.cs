...
namespace FaTestSoft
{
    ...
    {
        public XMLCfg(ToolStripMenuItem xML配置视图ToolStripMenuItem)
        {
            InitializeComponent();
            this.xML配置视图ToolStripMenuItem = xML配置视图ToolStripMenuItem;
        }
        ToolStripMenuItem xML配置视图ToolStripMenuItem;
        TabPage[] tp;
        int rownum = 0;
       ...
       //     if (node.NodeType == XmlNodeType.Text)
        ...
        private void XMLCfg_DockStateChanged(object sender, EventArgs e)
        {
            if (this.Visible == true)
                xML配置视图ToolStripMenuItem.Checked = true;
            else
                xML配置视图ToolStripMenuItem.Checked = false;
        }
    }
}
